	...
	private recieve(m: ServerMessage): void {
		switch (m.type) {
					...
					},
					{
						...
						click: () => {
							GameMasterApp.getInstance().client.send({
								...
								member_id: m.id,
								chips: Number.parseInt(
									(
										document.getElementById(
											"chips_" + m.id.toString()
										) as HTMLInputElement
									).value
								),
								einsatz: Number.parseInt(
									(
										document.getElementById(
											"einsatz_" + m.id.toString()
										) as HTMLInputElement
									).value
								),
							});
							console.log(
								"Es wurde die Values geupdated von einem der Clients"
							);
						},
					},
					{
			...
			case ServerEvents.REMOVED_MITGLIED:
				GameMasterApp.getInstance().teilnehmerTable.deleteRowById(
					m.id.toString()
				);
				break;

			case ServerEvents.UPDATED_MITGLIED_VALUES:
				(document.getElementById("chips_" + m.id) as HTMLInputElement).value =
					m.chips.toString();
				(document.getElementById("einsatz_" + m.id) as HTMLInputElement).value =
					m.einsatz.toString();

				if (!m.hasControls || m.status == MemberStatus.FOLDED) {
					GameMasterApp.getInstance().teilnehmerTable.unhighlightRowByValue(
						m.id.toString()
					);
				}

				if (m.status == MemberStatus.PLEITE) {
					GameMasterApp.getInstance().teilnehmerTable.editRowValueByValue(
						m.id.toString(),
						"Kontrolle",
						"Pleite",
						true
					...
					GameMasterApp.getInstance().teilnehmerTable.highlightRowByValue(
						m.id.toString(),
						HighlightColor.FOLDED
					);
				}
