...
export default class GameManager {
	...
	public getRemainingChips(userId: number): number {
		return (
			Cache.getInstance().getClientCacheById(userId)?.chips ??
			10000 - this.getBetValues(userId)
		);
	}
	...
	private getNextPlayerById(lastPlayer: number): number {
		...
		do {
			...
			if (
				Cache.getInstance().getClientCacheById(currentId)!.chips > 0 &&
				Cache.getInstance().getClientCacheById(currentId)!.status ==
					MemberStatus.ON
			) {
