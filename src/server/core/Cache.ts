...
export default class Cache {
	...
	public getSchaetzung(memberId: number): number {
		if (
			this.schaetzungen.filter((x) => x[0] == memberId)[0] == null ||
			this.schaetzungen.filter((x) => x[0] == memberId)[0] == undefined
		) {
			return 0;
		}
	...
	public updateClient(id: number, client: ClientCache): void {
		this.clients[
			this.clients.indexOf([id, this.getClientCacheById(id) as ClientCache])
		] = [id, client];
	}
